...
const viewSalesReport = () => {
  ...
  const query = db.query("SELECT department_id, department_name, AVG(overhead_costs) AS overhead, sum(product_sales) AS total_sales, (sum(product_sales) - overhead_costs) AS total_profit FROM departments JOIN products ON department_name = dept_name GROUP BY department_id, department_name", (err, deptData) => {
